...
export const gray =
  "bg-[tan] contrast-[0.9] transition-none";
export const silver =
  "bg-[wheat] contrast-[0.9] transition-none";
export const cellRevealed =
  "animate-[cellReveal_0.10s_ease-out] transition-none";
export const flagged =
  "animate-[flagFall_0.15s] transition-none";
export const bomb =
  "animate-[bombExplode_0.4s] transition-none";
export const cellWin =
  "animate-[cellWin_0.6s_ease-out_forwards]";
export const cellWinContent =
  "animate-[cellWinContent_0.6s_ease-out_forwards]";
...
function CellComponent({ id }: { id: number }) {
  ...
  const isHiddenOrFlagged =
    cellData.status === "hidden" || cellData.status === "flagged" || cellData.value === "bomb";
  ...
  const winningContent = (() => {
    ...
    if (typeof cellData.value === "number" && cellData.value > 0) return cellData.value;
    return null;
  })();

  // Add revealed class if cell is revealed
  const revealedClass = (cellData.status === "revealed" && cellData.value !== "bomb") ? cellRevealed : "";

  const numberColor = cellData.value === "bomb" ? "text-red-600" : getNumberColor(cellData.value);

  const size = typeof content === "number" ? ("text-lg font-medium " + getNumberColor(content)) : "";
    ...
    if (cellData.status === "flagged") {
      return (
        <span
          ...
          className={flagged}
          style={{ zIndex: 10000, position: 'relative', display: 'inline-block', pointerEvents: 'none', fontSize }}
        >
          {content}
        </span>
      )
    ...
    if (isWinning && winningContent) {
      return (
        <span
          ...
          style={{ position: 'relative', display: 'inline-block', pointerEvents: 'none' }}
        >
          {winningContent}
        </span>
      )
  ...
  return (
    <div
      className={cn(
        ...
        numberColor,
        isWinning && cellWin,
        `box-border w-[35px] h-[35px] flex items-center justify-center select-none relative text-[23px] font-bold`
      )}
      style={isWinning ? { "--win-color": winColor } as React.CSSProperties : undefined}
